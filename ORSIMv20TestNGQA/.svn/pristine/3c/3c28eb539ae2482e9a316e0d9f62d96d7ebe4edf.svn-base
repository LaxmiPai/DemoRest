package com.oracle.core.reports;
 ...
 * Created by VISISHTA.
 */
public class TestNGCustomReportListener extends MyReporterListenerAdapter {
	...
	protected PrintWriter createWriter(String outdir) throws IOException {
		...
		Date dt = new Date();
		String testNGReport = "testNGReport";
		outdir = outdir + File.separator + testNGReport;
		new File(outdir).mkdirs();
		return new PrintWriter(
				new BufferedWriter(new FileWriter(new File(outdir, "emailable-report" + now.format(dt) + ".html"))));
	}
		...
		if (tests.getAllResults().size() > 0) {
			...
			for (ITestNGMethod method : getMethodSet(tests, suite)) {
				...
				buff.append("<td><a href=\"#m" + m_methodIndex + "\">" + qualifiedName(method) + " "
						...
						+ "<td class=\"numi\">" + resultSet.size() + "</td>" + "<td class=\"numi\">"
						+ ((end - start) / 1000.) / 60. + "</td>" + "</tr>");
			}
			if (mq > 0) {
				cq += 1;
				m_out.print("<tr class=\"" + style + (cq % 2 == 0 ? "even" : "odd") + "\">" + "<td");
	...
	private void startResultSummaryTable(String style) {
		...
		m_out.println("<tr><th>Class</th>" + "<th>Method</th><th># of<br/>Scenarios</th><th>Time<br/>(ms)</th></tr>");
		m_row = 0;
	}
	...
	//	old code in Test NG 6.14.3
	//	m_out.print(Utils.shortStackTrace(exception, true)[0]);
	...
	public void generateSuiteSummaryReport(List<ISuite> suites) {
		...
		for (ISuite suite : suites) {
			...
			for (ISuiteResult r : tests.values()) {
				...
				// NEW
				summaryCell(overview.getStartDate().toGMTString(), true);
				m_out.println("</td>");
				summaryCell(overview.getEndDate().toGMTString(), true);
				m_out.println("</td>");
